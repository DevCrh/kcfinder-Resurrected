    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        if (_.access.files.copy || _.access.files.move) {
            ...
            _.menu.addItem("kcact:clpbrdadd", _.label("Add to Clipboard"), function () {
                ...
                _.initClipboard();
                if (msg.length) _.alert(msg.substring(0, msg.length - 1));
                return false;
            });
        }
        ...
        if (_.access.files['delete']) {
            ...
            _.menu.addItem("kcact:rm", _.label("Delete"), function () {
                ...
                if (failed)
                    _.confirm(
                        _.label("{count} selected files are not removable. Do you want to delete the rest?", {
                            count: failed
                        }),
                        go
                    );
        ...
        // DOWNLOAD
        _.menu.addItem("kcact:download", _.label("Download"), function () {
            $('#menu').html('<form id="downloadForm" method="post" action="' + _.getURL('download') + '"><input type="hidden" name="directory" /><input type="hidden" name="file" /><input type="hidden" name="csrf_token" value="' + csrfToken + '" /></form>');
...
_.menuDir = function (dir, e) {
    ...
    if (_.clipboard && _.clipboard.length) {
        ...
        if (_.access.files.copy)
            _.menu.addItem("kcact:cpcbd", _.label("Copy {count} files", {
                count: _.clipboard.length
            }), function () {
        ...
        if (_.access.files.move)
            _.menu.addItem("kcact:mvcbd", _.label("Move {count} files", {
                count: _.clipboard.length
            }), function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            dir.parent().hide(500, function () {
                                ...
                                if (pDir.data('path') == _.dir.substring(0, pDir.data('path').length))
                                    _.changeDir(pDir);
                                _.initDropUpload();
                            });
                        },
